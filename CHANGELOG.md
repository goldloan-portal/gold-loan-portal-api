...
- [GLA-16] `GET /api/v1/leads` — returns all `Lead` rows ordered by `createdAt` desc, each with a masked `mobileNumber` (e.g. `9876XXXX10`, via a pure `maskMobileNumber` in `src/services/lead.util.ts`), the calculated `maxEligibleLoan`, and the nested `selectedPlan` as `plan: { id, name }`.
- [GLA-21] `POST /api/v1/leads/calculate` — stateless preview endpoint reusing the same `calculatePureGoldWeight`/`calculateMaxEligibleLoan` logic and net-vs-gross validation as `POST /leads/submit`, returning `{ pureGoldWeight, maxEligibleLoan }` for the frontend's live loan calculator without persisting anything. New `calculateLeadSchema` shares its weight/purity fields with `createLeadSchema` to avoid drift.
- [GLA-30] `AI_LOG.md`'s required sections finalized (second exact prompt filled in, flawed-code section cross-references the real instance logged in the sibling frontend repo rather than fabricating a backend-specific one) — covers GLA-31. `README.md` gains a "Running the Full Stack Locally" section cross-referencing the sibling `gold-loan-portal-frontend` repo, and a "Development Process" section noting the Jira-driven Agile workflow (project key `GLA`).
